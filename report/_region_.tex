\message{ !name(report.tex)}\documentclass[pageno]{jpaper}
...
before desugaring the \lsti{let} into \lsti{\ (x:int) => x + 24) 5}. I chose not to do this because, again I plan at some point to implement type inference
...
Then the equivalence rules are:

\begin{prooftree}
\AxiomC{}
\UnaryInfC{Typerec int of $(\mu_i|\mu_{\times}|\mu_{\rightarrow}) \equiv$
\end{prooftree}


Consider the product branch of the \lsti{Typerec}. In my original method,
after being parsed, the branch would be transformed into the type operator
\lsti[mathescape]{\\($\tau_1$::*) ($\tau_2$::*) => RecArray[$\tau_1$] * RecArray[$\tau_2$]}. 
There are two recursive calls to \lsti{RecArray} here.
Now instead what we do is we replace the calls to \lsti{RecArray} with two fresh variables and add
them as parameters to our type-operator, like so:
\lsti[mathescape]{\\$\tau_1$ $\tau_2$ $\rho_1$ $\rho_2$ => $\rho_1$ * $\rho_2$}

So now, when we are evaluating something like
\begin{lstlisting}[mathescape]
Typecase $int * bool$ of
| bool => boolarray
| int => intarray
| $\tau_1 * \tau_2$ => (rec \\RecArray ...)[$\tau_1$] *
            (rec \\RecArray ...)[$\tau_2$]
...
\message{ !name(report.tex) !offset(-1168) }
