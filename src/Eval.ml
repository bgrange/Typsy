...
exception BadUnop of unop * exp
exception BadBinop of exp * binop * exp 
exception BadPair of exp
exception BadTCase of exp

let apply_unop (op:unop) (v:exp) : exp =
  match op, v with
  | StrLen, Constant (Str s) -> Constant (Int (String.length s))
  | _ -> raise (BadUnop (op,v)) 

let apply_binop (v1:exp) (op:binop) (v2:exp) : exp =
  match v1, op, v2 with 
  ...
  | Constant (Int i), LessEq, Constant (Int j) -> 
    Constant (Bool (i<=j))
  | Constant (Int i), Gt, Constant (Int j) -> 
    Constant (Bool (i<j))
  | Constant (Int i), GtEq, Constant (Int j) -> 
    Constant (Bool (i<=j))
  | Constant (Str s1), Concat, Constant (Str s2) ->
    Constant (Str (s1 ^ s2))
  | Constant (Str s), CharAt, Constant (Int i) ->
    Constant (Str (String.make 1 (String.get s i)))
  | Constant (Str s1), StrEq, Constant (Str s2) ->
    Constant (Bool (s1 = s2))
  ...
  | Constant (Bool b1), Or, Constant (Bool b2) -> 
    Constant (Bool (b1 || b2))
  | Constant (Bool b1), BoolEq, Constant (Bool b2) ->
    Constant (Bool (b1 = b2))
  | _, _, _ -> raise (BadBinop (v1,op,v2))
...
let eval_body (env:env) (tenv:tenv)
    (eval_loop:env -> tenv -> exp -> exp) (e:exp) : exp =
